  ...
  factory RoomSearchResponseDto.fromJson(Map<String, dynamic> json) {
    ...
    return RoomSearchResponseDto(
      query: RoomSearchRequest(
        roomPrefixes: List<String>.from(
          queryJson['room_prefixes'] ?? const <String>[],
        ),
        date: queryJson['date'] as String,
        since: queryJson['since'] as String?,
        until: queryJson['until'] as String?,
        buildingCodes: List<String>.from(
          queryJson['building_codes'] ?? const <String>[],
        ),
        utilities: List<String>.from(
          queryJson['utilities'] ?? const <String>[],
        ),
        nearMe: queryJson['near_me'] as bool? ?? false,
        userLocation: queryJson['user_location'] == null
            ? null
            : SearchLocation(
                latitude: (queryJson['user_location']['latitude'] as num)
                    .toDouble(),
                longitude: (queryJson['user_location']['longitude'] as num)
                    .toDouble(),
              ),
      ...
      items: (json['items'] as List<dynamic>? ?? const [])
          .map(
            (e) =>
                RoomSearchItemDto.fromJson(Map<String, dynamic>.from(e as Map)),
          )
          .toList(),
    );
  }
  ...
  factory RoomSearchItemDto.fromJson(Map<String, dynamic> json) {
    return RoomSearchItemDto(
      ...
      matchingWindows: (json['matching_windows'] as List<dynamic>? ?? const [])
          .map(
            (e) =>
                MatchingWindowDto.fromJson(Map<String, dynamic>.from(e as Map)),
          )
          .toList(),
...
class MatchingWindowDto {
  const MatchingWindowDto({required this.start, required this.end});
  ...
  MatchingWindow toDomain() => MatchingWindow(start: start, end: end);
}
